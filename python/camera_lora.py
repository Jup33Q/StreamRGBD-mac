...
Usage:
    ...
    # Custom LoRA path
    python camera_lora.py --prompt "anime style" --lora-path /path/to/custom.safetensors
...
class LoRAEnhancedPipeline:
    ...
    def __init__(self, model_name, render_size, output_size, prompt,
        ...
        enc_path = self._ensure_vae_encoder(render_size, coreml_dir)
        dec_path = self._ensure_vae_decoder(render_size, coreml_dir)
...
class CameraApp:
    ...
    def run(self):
        ...
        win = f"StreamDiffusion-LoRA ({self.pipeline.model_name} {self.pipeline.render_size})"
